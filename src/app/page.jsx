...
// ══ Supabase DB ══════════════════════════════════════════════
const db = {
  async getAll(table, map) {
    const { data, error } = await supabase.from(table).select("*").order("created_at", { ascending: table !== "purchases" && table !== "dining_out" && table !== "cook_history" });
    if (error) throw error;
    return (data || []).map(map);
  },
  ing: {
    async get() { return db.getAll("ingredients", r=>({id:r.id,name:r.name,unit:r.unit,category:r.category,stock:r.stock})); },
    async add(i) { await supabase.from("ingredients").insert({id:i.id,name:i.name,unit:i.unit,category:i.category,stock:i.stock}); },
    async updateStock(id,stock) { await supabase.from("ingredients").update({stock}).eq("id",id); },
    async del(id) { await supabase.from("ingredients").delete().eq("id",id); },
  },
  rcp: {
    async get() { return db.getAll("recipes", r=>({id:r.id,name:r.name,plate211:r.plate211,ingredients:r.ingredients})); },
    async add(r) { await supabase.from("recipes").insert({id:r.id,name:r.name,plate211:r.plate211,ingredients:r.ingredients}); },
    async del(id) { await supabase.from("recipes").delete().eq("id",id); },
  },
  pur: {
    async get() { return db.getAll("purchases", r=>({id:r.id,ingId:r.ing_id,ingName:r.ing_name,ingUnit:r.ing_unit,purchaseUnit:r.purchase_unit,qty:r.qty,baseQty:r.base_qty,price:r.price,date:r.date})); },
    async add(p) { await supabase.from("purchases").insert({id:p.id,ing_id:p.ingId,ing_name:p.ingName,ing_unit:p.ingUnit,purchase_unit:p.purchaseUnit,qty:p.qty,base_qty:p.baseQty,price:p.price,date:p.date}); },
    async del(id) { await supabase.from("purchases").delete().eq("id",id); },
  },
  din: {
    async get() { return db.getAll("dining_out", r=>({id:r.id,name:r.name,type:r.type,price:r.price,date:r.date,note:r.note,plate211:r.plate211})); },
    async add(d) { await supabase.from("dining_out").insert({id:d.id,name:d.name,type:d.type,price:d.price,date:d.date,note:d.note,plate211:d.plate211}); },
    async del(id) { await supabase.from("dining_out").delete().eq("id",id); },
  },
  cook: {
    async get() { return db.getAll("cook_history", r=>({id:r.id,recipeId:r.recipe_id,recipeName:r.recipe_name,mtype:r.mtype,day:r.day,date:r.date,cost:r.cost,plate211:r.plate211})); },
    async add(h) { await supabase.from("cook_history").insert({id:h.id,recipe_id:h.recipeId,recipe_name:h.recipeName,mtype:h.mtype,day:h.day,date:h.date,cost:h.cost,plate211:h.plate211}); },
  },
  plan: {
    async get() {
      const { data } = await supabase.from("week_plan").select("*");
      const plan={};
      (data||[]).forEach(r=>{ plan[r.week_key]={slots:r.slots}; });
      return plan;
    },
    async set(key,slots) { await supabase.from("week_plan").upsert({week_key:key,slots,updated_at:new Date().toISOString()},{onConflict:"week_key"}); },
  },
};
...
const SEED_ING = [
  ...
  { id: "i7",  name: "番茄",   unit: "g",  category: "veggie",  stock: 0 },
  { id: "i8",  name: "空心菜", unit: "g",  category: "veggie",  stock: 0 },
  { id: "i9",  name: "雞腿",   unit: "隻", category: "protein", stock: 0 },
  { id: "i10", name: "高麗菜", unit: "g",  category: "veggie",  stock: 0 },
  { id: "i11", name: "魚",     unit: "條", category: "protein", stock: 0 },
  { id: "i12", name: "玉米",   unit: "根", category: "veggie",  stock: 0 },
  { id: "i13", name: "排骨",   unit: "g",  category: "protein", stock: 0 },
];
const SEED_RCP = [
  ...
  { id: "r4", name: "番茄炒蛋",        ingredients: [{ id: "i4", qty: 2 }, { id: "i7", qty: 200 }],                           plate211: { veggie: true,  protein: true,  carb: false } },
  { id: "r5", name: "蒜炒空心菜",      ingredients: [{ id: "i8", qty: 300 }],                                                  plate211: { veggie: true,  protein: false, carb: false } },
  { id: "r6", name: "滷雞腿飯",        ingredients: [{ id: "i9", qty: 2 }, { id: "i3", qty: 100 }],                           plate211: { veggie: false, protein: true,  carb: true  } },
  { id: "r7", name: "炒高麗菜",        ingredients: [{ id: "i10", qty: 300 }],                                                 plate211: { veggie: true,  protein: false, carb: false } },
  { id: "r8", name: "豆腐蛋花湯",      ingredients: [{ id: "i5", qty: 1 }, { id: "i4", qty: 2 }],                             plate211: { veggie: false, protein: true,  carb: false } },
  { id: "r9", name: "蒸魚糙米飯",      ingredients: [{ id: "i11", qty: 1 }, { id: "i3", qty: 100 }],                          plate211: { veggie: false, protein: true,  carb: true  } },
  { id: "r10", name: "玉米排骨湯",     ingredients: [{ id: "i12", qty: 1 }, { id: "i13", qty: 200 }],                         plate211: { veggie: true,  protein: true,  carb: false } },
  { id: "r11", name: "蒜炒花椰菜雞肉", ingredients: [{ id: "i6", qty: 200 }, { id: "i1", qty: 150 }, { id: "i3", qty: 100 }], plate211: { veggie: true,  protein: true,  carb: true  } },
];
...
export default function App() {
  ...
  useEffect(() => {
    Promise.all([db.ing.get(), db.rcp.get(), db.pur.get(), db.din.get(), db.cook.get(), db.plan.get()])
      .then(([ing,rcp,pur,din,cook,plan]) => {
        setIngredients(ing); setRecipes(rcp); setPurchases(pur);
        setDiningOut(din); setCookHistory(cook); setWeekPlan(plan);
        setLoading(false);
      }).catch(()=>setLoading(false));
  }, []);

  const getSlots = (key) => weekPlan[key]?.slots || [{ rid: "", cooked: false }];
  const setSlots = useCallback(async (key, slots) => {
    setWeekPlan(p => ({ ...p, [key]: { slots } }));
    await db.plan.set(key, slots);
  }, []);
  ...
  const markCooked = async (key, slotIdx) => {
    ...
    const updatedIng = ingredients.map(ing => {
      const used = r.ingredients.find(ri => ri.id === ing.id);
      if (!used) return ing;
      const ns = Math.max(0, ing.stock - used.qty);
      db.ing.updateStock(ing.id, ns);
      return { ...ing, stock: ns };
    });
    setIngredients(updatedIng);
    const newSlots = slots.map((s, i) => i === slotIdx ? { ...s, cooked: true } : s);
    await setSlots(key, newSlots);
    const { total } = calcRecipeCost(r, ingredients, purchases);
    const h = { id: uid(), recipeId: r.id, recipeName: r.name, mtype: parts[1], day: parts[0], date: today(), cost: total ?? 0, plate211: r.plate211 };
    setCookHistory(prev => [h, ...prev]);
    await db.cook.add(h);
  };

  const clearWeekPlan = async () => {
    // delete all week_plan rows from supabase
    await supabase.from("week_plan").delete().neq("week_key", "");
    setWeekPlan({});
  };
  ...
  if (loading) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:"#F7F4EE",flexDirection:"column",gap:16}}>
      <div style={{fontSize:48}}>🥗</div>
      <div style={{fontSize:16,color:"#3D6B4F",fontWeight:700,fontFamily:"sans-serif"}}>載入中...</div>
    </div>
  );
  ...
  return (
    <div style={{ fontFamily: "'Noto Sans TC', sans-serif", background: C.bg, minHeight: "100vh", color: C.text }}>
      ...
      <div style={{ padding: 14, maxWidth: 640, margin: "0 auto" }}>
        ...
        {tab === 3 && <WeekPlanTab weekPlan={weekPlan} getSlots={getSlots} setSlots={setSlots} recipes={recipes} setRecipes={setRecipes} ingredients={ingredients} setIngredients={setIngredients} purchases={purchases} weekCost={weekCost} weekBudget={weekBudget} savings={savings} shoppingList={shoppingList} onExport={exportShopping} markCooked={markCooked} clearWeekPlan={clearWeekPlan} />}
...
function IngredientsTab({ ingredients, setIngredients }) {
  ...
  const add = async () => {
    ...
    const item = { id: uid(), ...form, stock: Number(form.stock) || 0 };
    setIngredients(p => [...p, item]);
    await db.ing.add(item);
    setForm({ name: "", unit: "g", category: "veggie", stock: "" });
    setShow(false);
  };
  const del = async (id) => { setIngredients(p=>p.filter(x=>x.id!==id)); await db.ing.del(id); };
...
function RecipesTab({ recipes, setRecipes, ingredients, setIngredients, purchases }) {
  ...
  const add = async () => {
    if (!form.name.trim()) return;
    const r = { id: uid(), name: form.name, plate211: form.plate211, ingredients: form.ingIds.map(id => ({ id, qty: Number(form.ingQty[id]) || 1 })) };
    setRecipes(p => [...p, r]);
    await db.rcp.add(r);
  ...
  const del = async (id) => { setRecipes(p=>p.filter(x=>x.id!==id)); await db.rcp.del(id); };
...
// ══ 週計劃 ══════════════════════════════════════════════════
function WeekPlanTab({ weekPlan, getSlots, setSlots, recipes, setRecipes, ingredients, setIngredients, purchases, weekCost, weekBudget, savings, shoppingList, onExport, markCooked, clearWeekPlan }) {
  const [view, setView] = useState("plan");
  const [confirmInfo, setConfirmInfo] = useState(null);
  const [showClearConfirm, setShowClearConfirm] = useState(false); // { key, slotIdx }
    ...
    <div>
      ...
      {showClearConfirm && (
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center",padding:24 }}>
          <div style={{ background:"#fff",borderRadius:16,padding:24,maxWidth:300,width:"100%",boxShadow:"0 8px 32px rgba(0,0,0,0.2)" }}>
            <div style={{ fontSize:36,textAlign:"center",marginBottom:8 }}>🔄</div>
            <div style={{ fontWeight:800,fontSize:16,textAlign:"center",marginBottom:6 }}>開始新的一週？</div>
            <div style={{ fontSize:12,color:C.muted,textAlign:"center",marginBottom:6 }}>本週計劃將會清空</div>
            <div style={{ fontSize:12,color:C.green,background:C.greenLight,borderRadius:8,padding:"8px 12px",textAlign:"center",marginBottom:20 }}>✅ 料理史與月紀錄不受影響</div>
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>
              <button onClick={()=>setShowClearConfirm(false)} style={{ padding:"11px",borderRadius:10,border:`1.5px solid ${C.border}`,background:"#fff",color:C.muted,fontWeight:700,fontSize:14,cursor:"pointer" }}>取消</button>
              <button onClick={async()=>{ await clearWeekPlan(); setShowClearConfirm(false); }} style={{ padding:"11px",borderRadius:10,border:"none",background:C.amber,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer" }}>確認清空</button>
            </div>
          </div>
        </div>
      )}
      ...
      {view === "plan" && (
        <>
          ...
          <div style={{ display:"flex",justifyContent:"flex-end",marginBottom:10 }}>
            <button onClick={()=>setShowClearConfirm(true)} style={{ fontSize:12,color:C.amber,background:"none",border:`1.5px solid ${C.amber}`,borderRadius:8,padding:"6px 12px",cursor:"pointer",fontWeight:700 }}>🔄 開始新的一週</button>
          </div>
          {totalMeals > 0 && (
...
function DiningOutTab({ diningOut, setDiningOut }) {
  ...
  const add = async () => {
    ...
    const d = { id: uid(), ...form, price: Number(form.price) };
    setDiningOut(p => [d, ...p]);
    await db.din.add(d);
    setForm({ name: "", type: "早餐", price: "", date: today(), note: "", plate211: { veggie: false, protein: false, carb: false } });
    setShow(false);
  };
  const del = async (id) => { setDiningOut(p=>p.filter(x=>x.id!==id)); await db.din.del(id); };
